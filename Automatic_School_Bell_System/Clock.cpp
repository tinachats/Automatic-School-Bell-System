...
#define MINUTE 60 * 1000L /* ms The L at the end tells the compiler we want a long integer */
#define TIMESPAN_DAY TimeSpan(1, 0, 0, 0)

/*
  The NVRAM is small region of memory on the RTC chip. It lets us store values that will 
  remain even when the power goes off. We use the NVRAM to store the alarm state, 
  hour and minute. These constants define where we store these values inside the NVRAM.
*/
